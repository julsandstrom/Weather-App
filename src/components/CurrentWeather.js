...
function CurrentWeather({ cityName, showNextFiveDays }) {
  ...
  useEffect(() => {
    ...
    // TODO: kolla om cityName innehåller otillåtna tecken

    const getCoordinates = async (cityName) => {
      const geocodingResponse = await fetch(
        `https://api.opencagedata.com/geocode/v1/json?q=${cityName}&key=01691c2651ba421ba07249273c194234`
      );
      ...
      {showNextFiveDays ? (
        <div className="forecast-container">
          <h2 className="h3-title">
            {cityName} |<small> Next 5 Days</small>
          </h2>
              ...
              .map((maxTemp, index) => (
                <li key={index} className="forecast-item">
                  ...
                  <span style={{ color: "red" }}>
                    Low {dailyWeather.temperature_2m_min[index + 1]}°C
                  </span>
                  <span style={{ color: "green" }}>High {maxTemp}°C</span>
                  <WeatherDetails
                    weatherCode={dailyWeather.weathercode[index + 1]}
                  />
                </li>
              ))}
          </ul>
